...
namespace DarksideLeasing.App.Queue
{
    public class DarkSideLeasingQueue(ILogger<DarkSideLeasingQueue> logger, ILeaseCalculatorService leaseCalculatorService)
    {
        ...
        public async Task<DailyMileageCalcResponse> ProcessQuote([ActivityTrigger] DailyMileageCalcRequest request, FunctionContext executionContext)
        {
            ...
            var response = await leaseCalculatorService.CalculateDailyMilage(request);
